...
class Niveau {
	...
	function __construct($idNiveau,$codeNiveau,$libNiveau,$duree,$cycle) {
	...
	public function setCycle($newCycle) {
		if(!empty($newCycle))
			$this->cycle = $newCycle;
	}
	...
	public static function create($idNiveau, $codeNiveau, $libNiveau, $duree, $cycle) {
		...
		if(!empty($select) && $select->rowCount() == 0){
			...
			$insert=$bdd->prepare("INSERT INTO niveau(idNiveau,codeNiveau,libelleNiveau,duree,cycle) VALUES(?,?,?,?,?)");
			$insert->execute(array($idNiveau,$codeNiveau,$libNiveau,$duree,$cycle));
			...
			fputs($eventsFile, date('d/m/Y H:i:s')." Ajout de niveau $idNiveau $codeNiveau $libNiveau $duree $cycle par $prof ".$_SESSION['name']." ".$_SESSION['firstname']." \n");
			fclose($eventsFile);
	...
	public static function read($idNiveau) {
		...
		if(!empty($aNiveau)) {
			$niveau = new Niveau($aNiveau[0][0], $aNiveau[0][1], $aNiveau[0][2], $aNiveau[0][3], $aNiveau[0][4]);
			return $niveau;
		}
		else {
			$_SESSION['alert'] = 'error';
	...
	public function update($newCodeNiveau, $newLibNiveau, $Newduree, $Newcycle) {
		...
		if($codeNiveau != $newCodeNiveau && !empty($select) && $select->rowCount() > 0) {
			...
			$update = $bdd->prepare("UPDATE niveau SET codeNiveau=?, libelleNiveau=?, duree=?, cycle=? WHERE idNiveau='$idNiveau' ");
			$update->execute(array($newCodeNiveau, $newLibNiveau, $Newduree,$Newcycle));

			include_once '../Models/ProfilUser.class.php';
			$prof = (ProfilUser::read($_SESSION['codeProfil']))->getLibProfil();
			$eventsFile = fopen("../Uploads/niveau.txt", "a+");
			fputs($eventsFile, date('d/m/Y H:i:s')." Modification de niveau ''$idNiveau $codeNiveau $libNiveau $duree $cycle'' en ''$newCodeNiveau $newLibNiveau $Newduree $Newcycle)'' par $prof ".$_SESSION['name']." ".$_SESSION['firstname']." \n");
			fclose($eventsFile);
	...
	public function delete() {
		...
		if(!empty($select) && $select->rowCount() == 0) {
			...
			$prof = (ProfilUser::read($_SESSION['codeProfil']))->getLibProfil();
			$eventsFile = fopen("../Uploads/niveau.txt", "a+");
			fputs($eventsFile, date('d/m/Y H:i:s')." Suppression de niveau ''$idNiveau $codeNiveau $libNiveau $duree $cycle'' par $prof ".$_SESSION['name']." ".$_SESSION['firstname']." \n");
			fclose($eventsFile);

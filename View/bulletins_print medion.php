    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <div class="col-sm-2">
                    <a href="../bulletins-liste-etudiants/<?= $idClas ?>"><button class="btn btn-info btn-sm btn-block" data-tog="tooltip" data-placement="bottom" title="Retour en arrière"><i class="fas fa-angle-double-left" aria-hidden="true"></i> Retour</button></a>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="col-12">
                    <div class="card table-responsive" id="bulletin">
                        <div class="card-body" style="margin-top: 120px;">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-5" style="width: 45%;">
                                    <p style="font-weight: bold;">
                                        ...
                                        <span style="text-decoration: underline;">Filière</span> : <?php echo (Filiere::read(Filiere::findFiliereId($dec[1])->getIdFiliere()))->getLibFiliere(); ?>
                                    </p>
                                </div>
                            </div>
                            <div class="row">
                            ...
                            <table id="table_classes" class="table table-bordered table-sm border-dark text-sm">
                            <thead>
                                <tr>
                                    <th scope="col" colspan="2" style="vertical-align:middle;text-align:center;">UE</th>
                            ...
                            <?php
                                ...
                                ?>
                                    <tr>
                                        <td colspan="6" class="text-center bg-dark" style="color: white;">SEMESTRE
                                            <?php
                                                $dec = explode("-", $laClasse->getCodeClasse());
                                                if($dec[0] == "LP") {
                                                    if($dec[2] == 1) {
                                ...
                                for($nat = 0; $nat < 4; $nat++):
                                $diff = "true";
                                //Le nombre d'ue ayant la même nature dans le semestre
                                $ues = UE::getUE_SemestreNature($sem['codeSemestre'], $nat);
                                $natNbECU = UE::getNbECU_SemestreNature($sem['codeSemestre'], $nat);
                                foreach ($ues as $ue) {
                                    
                                    $nbr = ECU::getUEECUNumber($ues[0]['idUE']);
                                    $ecus = ECU::getUEECU($ues[0]['idUE']);
                                    $moyUE = (UE::read($ues[0]['idUE']))->getMoyUE($idEtu);
                                    if($ues[0]['codeTypeUE'] == "TC") {
                            ...
                            ?>
                                <tr>
                                    ...
                                    if($diff=="true"):
                                    ?>
                                    <td scope="col" rowspan="<?php echo $natNbECU ?>" class="text-sm text-center font-weight-bold" style="white-space: initial; vertical-align: middle; margin: 0px;">
                                        <p style="writing-mode:vertical-rl; transform:rotate(-180deg); font-size: 8px; height: <?php echo $natNbECU*22 ?>px; word-break: break-all; padding: 0px; margin: 0px;">
                                        <?php
                                            if($ues[0]['natureUE']==1) {
                                                echo "Unités de connaissance fondamentale";
                                            }
                                            else if($ues[0]['natureUE']==2) {
                                                echo "Unités de spécialité ou de découverte";
                                            }
                                            else if($ues[0]['natureUE']==3) {
                                                echo "Unités de méthodologie";
                                            }
                                            else if($ues[0]['natureUE']==4) {
                                                echo "Unités de culture générale";
                                            }
                                        ?>
                                        </p>
                                    </td>
                                    ...
                                    $diff = "false";
                                    ?>
                                    <th scope="col" rowspan="<?php echo $nbr ?>" class=" text-sm" style="vertical-align:middle;text-align:center;"><?php echo $ues[0]['codeUE'] ?></th>
                                    <td style="vertical-align: middle;"><?php echo $ecus[0]['libelleECU'] ?></td>
                                    <td style="vertical-align:middle;text-align:center;"><?php echo number_format(((ECU::read($ecus[0]['idECU']))->getMoyECU($idEtu, $ues[0]['idUE'], $idClas)), 2); ?></td>
                                    <td rowspan="<?php echo $nbr ?>" style="vertical-align:middle;text-align:center;"><?php echo ($ues[0]['coef'] < 10) ? "0".$ues[0]['coef'] : $ues[0]['coef']; $totalCredits+=$ues[0]['coef']; ?></td>
                                    <td rowspan="<?php echo $nbr ?>" style="vertical-align:middle;text-align:center;"><?php echo ($moyUE>=$validation)? "Validé" : "Non validé"; ($moyUE>=$validation)? $creditsObtenus+=$ues[0]['coef'] : $creditsObtenus+=0; ?></td>
                                </tr>
                                <?php
                                    $i = 0;
                                    $moyenneUECoef = 0.0;
                                    $moyenneUE = 0;
                                    foreach($ecus as $ecu):
                                        $moyECU = number_format(((ECU::read($ecu['idECU']))->getMoyECU($idEtu, $ues[0]['idUE'], $idClas)), 2);
                                        $moyenneUE += $moyECU;
                                        if($i != 0):
                                ?>
                                        <tr>
                                            <td style="vertical-align: middle;"><?php echo $ecu['libelleECU'] ?></td>
                                ...
                                <?php
                                    ...
                                    $i++;
                                    endforeach;
                                    $moyenneUECoef = ($moyenneUE/$nbr)*$ues[0]['coef'];

    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <div class="col-sm-2">
                    <a href="../proces-verbaux"><button class="btn btn-info btn-sm btn-block" data-tog="tooltip" data-placement="left" title="Gestion des notes"><i class="fas fa-angle-double-left" aria-hidden="true"></i> Retour</button></a>
                </div>
            </div>
        </div>
    </div>
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                                ...
                                <?php
                                    ...
                                    foreach($etudiants as $etudiant) {
                                        if($etudiant['idClasse']  == $idClasse ){
                                            ...
                                            if($etudiant['codeStatut'] == "RDBT") {
                                                //Récupérer l'idEtudiant de l'année précédente
                                                if(AnneeAcademique::anneePassee()) {
                                                    $idEtuPrecedent = (Etudiant::findMatriculeEtudiant($etudiant['matricule'], AnneeAcademique::anneePassee()->getIdAnnee()))->getIdEtudiant();
                                                }
                                            }
                                ?>
                                    <tr>
                                        ...
                                        $moyenne = 0.0;
                                            foreach($ues as $ue):
                                                ...
                                                if($etudiant['codeStatut'] == "RDBT") {
                                                    if(AnneeAcademique::anneePassee()){
                                                    //Récupérer l'idUE de l'année précédente
                                                    $idUEPrecedent = (UE::findCodeueUE($ue['codeUE'], AnneeAcademique::anneePassee()->getIdAnnee()))->getIdUE();

                                                    //Il faut si l'étudiant validait l'UE, si oui on récupère ses anciennes notes avec pour EF EFA

                                                    $classeObject = Classe::read($idClasse);
                                                    $idClassePrecedent = (Classe::findCodeClasseObject($classeObject->getCodeClasse(), AnneeAcademique::anneePassee()->getIdAnnee()))->getIdClasse();
                                                    
                                                    
                                                    $classeObjectPrecedent = Classe::read($idClassePrecedent);
                                                    $lUEPrecedent = UE::read($idUEPrecedent);
                                                    $moyUEPrecedent = $lUEPrecedent->getMoyUE($idEtuPrecedent);
                                                    unset($efaPrecedent);

                                                    if($lUEPrecedent->getCodeTypeUE() == "TC") {
                                                        $validationPrecedent = $classeObjectPrecedent->getValidationTC();
                                                    }
                                                    else {
                                                        $validationPrecedent = $classeObjectPrecedent->getValidationSP();
                                                    }
                                                    if($moyUEPrecedent >= $validationPrecedent) {
                                                        $dejaValide = true;
                                                    }

                                                    if($validationPrecedent != 12 && $dejaValide && $moyUEPrecedent < 12) {
                                                        
                                                        $efaPrecedent = $lUEPrecedent->ajusterUE($idClassePrecedent, $idEtuPrecedent);
                                        ...
                                        <?php
                                            ...
                                            foreach($ecus as $ecu):
                                                if($etudiant['codeStatut'] == "RDBT") {
                                                    //Récupérer l'idECU de l'année précédente
                                                    if(AnneeAcademique::anneePassee()) {
                                                        $idECUPrecedent = (ECU::findCodeecuECU($ecu['codeECU'], AnneeAcademique::anneePassee()->getIdAnnee()))->getIdECU();
                                                    }
                                                }
                                        ?>
                                            <td style="vertical-align:middle;text-align:center;">
                                                <?php
                                                    if($dejaValide) {
                                                        echo $cc = Evaluation::getEvalNote("CC", $idECUPrecedent, $idEtuPrecedent);
                                                    }
                                                    else {
                                                    ...
                                                    if(empty($cc)) {
                                                        if($dejaValide==true) {
                                                            if(isset($efaPrecedent)){
                                                                echo $cc = number_format(($efaPrecedent[$numECU]), 2);
                                                            }
                                                            else {
                                                                echo $cc = Evaluation::getEvalNote("EF", $idECUPrecedent, $idEtuPrecedent);
                                                            }
                                                        }
                                                        else {
                                            ...
                                            <td style="vertical-align:middle;text-align:center;">
                                                <?php
                                                    if($dejaValide) {
                                                        if(isset($efaPrecedent)) {
                                                            echo $ef = number_format(($efaPrecedent[$numECU]), 2);
                                                        }
                                                        else {
                                                            echo $ef = Evaluation::getEvalNote("EF", $idECUPrecedent, $idEtuPrecedent);
                                                        }
                                                    }
                                                    else {
                                        ...
                                        <td>
                                            <?php
                                                ...
                                                if($defaillant) {
                                                    ...
                                                    if($class[2] == $duree) {
                                                        if($etudiant['sexe']=="M") {
                                                            if($pourcent == 100): echo "ADMISSIBLE"; else: echo "REFUSE"; endif;
                                                        }
                                                        else {
                                                        ...
                                                        if($etudiant['sexe']=="M") {
                                                            if($pourcent == 100): echo "ADMIS SANS REPRISE"; elseif($pourcent >= 85): echo "ADMIS AVEC REPRISE";  else: echo "REFUSE"; endif;
                                                        }
                                                        else {
                                                            if($pourcent == 100): echo "ADMISE SANS REPRISE"; elseif($pourcent >= 85): echo "ADMISE AVEC REPRISE";  else: echo "REFUSE(E)"; endif;
                                                        }
                                                    }
                                                }
                ...
                <!-- ***************************************Proces verbal ajusté************************************ -->
                <div class="row">
                <div class="col-12">
                    <h4 class="text-center w-full py-2 text-white font-weight-bold" style="background-color: #044687;">
                        Procès verbal ajusté
                    </h4>
                    <div class="card table-responsive">
                        <div class="card-body">
                            <style>
